...
public class DeliveryListener implements Listener {
    ...
    public void onHurt(EntityDamageEvent damageEvent) {
        if (Settings.DELIVERY_MOB_INVINCIBLE.getBoolean()) {
            boolean invincible = _PrivateReserve.DELIVERY_DATA.isInvincible(damageEvent.getEntity());
            if (invincible) {
    ...
    public void onMove(EntityMoveEvent event) {
        if (_PrivateReserve.DELIVERY_DATA.isFrozen(event.getEntity())) {
    ...
    public void onControl(VehicleEnterEvent event) {
        if (_PrivateReserve.DELIVERY_DATA.isActive(event.getVehicle())) {
            event.setCancelled(true);
        }
    }
}
